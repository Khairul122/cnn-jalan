"""skema awal (Postgres/Supabase)

Revision ID: d82f9fbc8d21
Revises: 
Create Date: 2026-09-24 08:39:03.955578
...
def upgrade():
    ...
    sa.Column('role', sa.Enum('admin', 'viewer', native_enum=False), nullable=False),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    ...
    sa.Column('sumber_data', sa.Enum('primer', 'sekunder', native_enum=False), nullable=False),
    ...
    sa.Column('resize_method', sa.Enum('LANCZOS', 'BILINEAR', 'BICUBIC', 'NEAREST', native_enum=False), nullable=False),
    sa.Column('resize_mode', sa.Enum('stretch', 'letterbox', native_enum=False), nullable=False),
    ...
    sa.Column('norm_method', sa.Enum('minmax', 'zscore', 'none', 'clahe', native_enum=False), nullable=False),
    sa.Column('denoise_method', sa.Enum('none', 'gaussian', 'median', 'bilateral', native_enum=False), nullable=False),
    ...
    sa.Column('label_sumber', sa.Enum('tingkat', 'jenis', native_enum=False), nullable=False),
    ...
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('nama', sa.String(length=100), nullable=False),
    sa.Column('model_type', sa.Enum('mobilenetv2', 'efficientnetb0', native_enum=False), nullable=False),
    ...
    sa.Column('optimizer', sa.Enum('adam', 'sgd', 'rmsprop', native_enum=False), nullable=False),
    ...
    sa.Column('status', sa.Enum('draft', 'training', 'selesai', 'gagal', native_enum=False), nullable=False),
    ...
    sa.Column('jenis_retak', sa.Enum('halus', 'lebar', native_enum=False), nullable=False),
    ...
    sa.Column('status', sa.Enum('selesai', 'gagal', native_enum=False), nullable=False),
    sa.Column('catatan', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    ...
    sa.Column('status_pemetaan', sa.Enum('draft', 'terverifikasi', 'diperbaiki', native_enum=False), nullable=False),
    ...
    # Data master (id eksplisit, sama dengan skema MySQL lama). id = indeks kelas CNN + 1 (lihat app/kelas.py).
    tingkat = sa.table('tingkat_kerusakan', sa.column('id'), sa.column('nama_tingkat'),
                       sa.column('warna_peta'), sa.column('skor_prioritas'))
    op.bulk_insert(tingkat, [
        {'id': 1, 'nama_tingkat': 'Rusak Berat', 'warna_peta': '#E53E3E', 'skor_prioritas': 1},
        {'id': 2, 'nama_tingkat': 'Rusak Ringan', 'warna_peta': '#F97316', 'skor_prioritas': 2},
        {'id': 3, 'nama_tingkat': 'Sedang', 'warna_peta': '#F59E0B', 'skor_prioritas': 3},
        {'id': 4, 'nama_tingkat': 'Baik', 'warna_peta': '#10B981', 'skor_prioritas': 4},
    ])
    jenis = sa.table('jenis_kerusakan', sa.column('id'), sa.column('nama_jenis'), sa.column('kode'), sa.column('deskripsi'))
    op.bulk_insert(jenis, [
        {'id': 1, 'nama_jenis': 'Retakan', 'kode': 'CRACK',
         'deskripsi': 'Retak halus, memanjang, melintang, atau retak buaya pada permukaan jalan'},
        {'id': 2, 'nama_jenis': 'Berlubang', 'kode': 'POTHOLE',
         'deskripsi': 'Kerusakan struktural membentuk cekungan akibat pengelupasan lapisan aspal'},
        {'id': 3, 'nama_jenis': 'Gelombang', 'kode': 'RUTTING',
         'deskripsi': 'Deformasi permukaan jalan yang tidak rata akibat pergeseran material'},
    ])
    # Insert dengan id eksplisit tidak memajukan sequence: samakan agar insert berikutnya tidak bentrok.
    op.execute("SELECT setval(pg_get_serial_sequence('tingkat_kerusakan', 'id'), 4)")
    op.execute("SELECT setval(pg_get_serial_sequence('jenis_kerusakan', 'id'), 3)")

    # Supabase mengekspos skema public lewat PostgREST (anon key). RLS aktif TANPA policy = anon/authenticated
    # tidak bisa membaca/menulis apa pun; aplikasi Flask memakai role pemilik (postgres) yang melewati RLS.
    for tabel in ('evaluasi_model', 'jenis_kerusakan', 'pengguna', 'tingkat_kerusakan', 'lokasi_kerusakan',
                  'preprocessing_config', 'split_config', 'arsitektur_config', 'dokumentasi_foto', 'label_kerusakan',
                  'hasil_evaluasi', 'hasil_klasifikasi_cnn', 'hasil_preprocessing', 'hasil_training',
                  'prediksi_model', 'split_item', 'peta_kerusakan', 'alembic_version'):
